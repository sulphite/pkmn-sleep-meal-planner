<script lang="ts">
  ...
  ingredients.forEach((item: Ingredient) => {
    ingredientCounts = {
      ...ingredientCounts,
      [item.Name]: { count: 0, strength: item.Strength },
    };
  });
  ...
  const filterByIngredients = (
    recipes: Recipe[],
    ingredientList: IngredientTotals
  ): Recipe[] => {
  ...
  $: filteredRecipes = {
    curries: filterByIngredients(recipes.curries, ingredientCounts).sort(
      sortFn
    ),
    salads: filterByIngredients(recipes.salads, ingredientCounts).sort(sortFn),
    desserts: filterByIngredients(recipes.desserts, ingredientCounts).sort(
      sortFn
    ),
  };
</script>
...
<main class="flex flex-col gap-4 py-4">
  ...
  {:else}
    <div in:fly={{x: -1000, delay: 500}} out:fly={{x: -1000, duration: 500}}>
      <TabGroup>
